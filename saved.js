...
functions.load_model('Cars.glb', '/assets/models/', scene, 0xfff0ff, {x:0.5, y:0.5, z:0.5}, {x:1, y:1, z:1}, function(loadModel){
    ...
    function car_controls(event) {
        ...
        let engine_force = 900, max_steer = 0.3;
        let brake_force = 5; // Define a high brake force
...
for (let i = 0; i < 4; i++) {
    ...
    let store_action;
    let store_mixer;
    functions.load_model_animation_base('store2.glb', '/assets/models/', scene, 0xfff0ff, {x:(0.5*i)+(10*i), y:0.5, z:0.5}, {x:2, y:2, z:2}, function(loadModel){
...
let tex_path = ['scroll_tex1', 'scroll_tex2', 'scroll_tex3', 'scroll_tex4', 'scroll_tex5', 'scroll_tex6'];
let texs = {};
    ...
    for (let j=1; j <= 6; j++){
        ...
        let lidMesh = container_model.getObjectByName('lid');
            ...
            if (lidMesh && lidMesh.material) {
                lidMesh.material.roughness = 1;
                lidMesh.material.color.setHex(0xA27F73); 
            }
        ...
        functions.load_model_animation('123.glb', '/assets/models/', scene, 0xc2e1e3, {x:render_pos.x, y:render_pos.y, z:render_pos.z}, {x:0.27, y:0.27, z:0.27}, function(loadModel){
            ...
            action.play();
            scrolls.push([scroll_model, action, mixer, false, render_pos, false]); // bool -> 1. rotation 2. open & close
            if (scrolls.length == 24){
                functions.quickSort2(scrolls, 0, scrolls.length-1)
            }
            
            if (texs[`scroll_tex${scroll_count}`] !== undefined){
                scroll_model.traverse((child) => {
                    if(child.isMesh){
...
let barrel_pos = [
    ...
    functions.create_phy_model_cylinder(new CANNON.Vec3(-42.7, 0.4, -3.9), barrel_radius, barrel_height, barrel_seg, 2, world),
...
function check_collision() {
    ...
    for (let i = 0; i < shop_models.length; i++){
...
document.addEventListener('click', (e) => {
    ...
    if (intersects.length > 0) {
        ...
        if (selectedContainer.position.z == (current_cam_pos.z-1) && lidMesh.position.y.toFixed(2) == 0.56){
            move_obj(selectedContainer, default_pos);
            move_obj(selected_scroll, default_pos);
            object_rotate = false;
        }
    }

    if (selectedContainer && selectedContainer.position.y.toFixed(1) === (current_cam_pos.y-2).toFixed(1) && scrolls[containers.indexOf(selectedContainer)][5]){
            ...
            if (intersect_scroll.length > 0){
                ...
                // __________________________________animation reset__________________________________ //
                
                scrolls[containers.indexOf(selectedContainer)][1].reset();
                scrolls[containers.indexOf(selectedContainer)][1].timeScale = -8;
                scrolls[containers.indexOf(selectedContainer)][1].time = scrolls[containers.indexOf(selectedContainer)][1].getClip().duration
                scrolls[containers.indexOf(selectedContainer)][1].play()

                scrolls[containers.indexOf(selectedContainer)][2].addEventListener('finished', function () {
                    scrolls[containers.indexOf(selectedContainer)][1].reset();
                    scrolls[containers.indexOf(selectedContainer)][1].timeScale = 1; 
                    scrolls[containers.indexOf(selectedContainer)][1].time = 0; // Set to the first frame
                    scrolls[containers.indexOf(selectedContainer)][1].paused = true; // Start paused
                    scrolls[containers.indexOf(selectedContainer)][1].setLoop(THREE.LoopOnce);
                    scrolls[containers.indexOf(selectedContainer)][1].clampWhenFinished = true;
...
// }
// </script>
// <script id="fragShader" type="fragment">
